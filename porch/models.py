...
class PorchTypicalProject(TypicalProject):
    height = models.DecimalField(
        'высота, м', max_digits=2, decimal_places=1, null=True)
    width = models.DecimalField(
        'ширина, м', max_digits=2, decimal_places=1, null=True)
    # length = models.DecimalField('длина, м', max_digits=2, decimal_places=1, null=True)
    depth = models.DecimalField(
        'глубина, м', max_digits=2, decimal_places=1, null=True)

    frame_kosour_count = models.PositiveSmallIntegerField(
        'количество косоуров', null=True)
    frame_material = models.CharField('материал каркаса', max_length=250)
    frame_width = models.PositiveSmallIntegerField(
        'ширина каркаса, мм', null=True)
    frame_thickness = models.PositiveSmallIntegerField(
        'толщина каркаса, мм', null=True)
    ...
    steps_height = models.DecimalField(
        'высота ступеней, м', max_digits=4, decimal_places=2, null=True)
...
class Porch(SimplePage, PageDescription, PageMenu, PagePortfolio, PageHWAW, PageRailings):
    ...
    def get_absolute_url(self):
        return reverse('porch:detail', kwargs={'slug': self.slug})
